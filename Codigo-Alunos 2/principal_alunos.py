...
def pacman_movimentos(estado_jogo, direcao):
    estado_jogo['pacman']['direcao_atual'] = direcao

def pacman_cima(estado_jogo):
    pacman_movimentos(estado_jogo, DIRECOES_POSSIVEIS[0])   

def pacman_baixo(estado_jogo):
    pacman_movimentos(estado_jogo, DIRECOES_POSSIVEIS[1])


def pacman_direita(estado_jogo):
    pacman_movimentos(estado_jogo, DIRECOES_POSSIVEIS[2])


def pacman_esquerda(estado_jogo):
    pacman_movimentos(estado_jogo, DIRECOES_POSSIVEIS[3]) 
...
def aproximar(posicaoPacman, posicaoFantasma):
    ...
    if abs(direcao[0]) > abs(direcao[1]):
        ...
        direcao[0] = 0
        direcao[1] = PIXEIS_MOVIMENTO if direcao[1] > 0 else -PIXEIS_MOVIMENTO

    if movimento_valido((posicaoFantasma[0] + direcao[0], posicaoFantasma[1] + direcao[1]), estado_jogo):
        return direcao

    melhorDirecao = (0, 0)
    menorDistancia = TAMANHO_CELULA ** 2
...
def movimenta_pinky(estado_jogo):
    ...
    objetoPinky = estado_jogo['fantasmas'][PINKY_OBJECT]['objeto']

    return aproximar(objetoPacman.pos(), objetoPinky.pos())
...
def movimenta_clyde(estado_jogo):
    ...
    if distancia > scatter_distance_threshold:
        ...
        return aproximar(posicaoClyde, scatter_corner_index)
...
def guarda_jogo(estado_jogo):
    ...
    with open('save.txt', 'w') as file:
        ...
        for i in range(0, len(estado_jogo['mapa']), TAMANHO_CELULA):
            ...
            str_mapa += ','.join(str(i) for i in linha) + ',\n'
        file.write(str_mapa)
